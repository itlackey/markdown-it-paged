...
For `@spread`, `@page`, and `@section`, bare tokens become ambiguous once you also add explicit attrs like `template=...` or `region=...`. In that form, the first bare token is treated as the marker name. If you want an unnamed marker with attrs plus classes, keep the classes explicit:
